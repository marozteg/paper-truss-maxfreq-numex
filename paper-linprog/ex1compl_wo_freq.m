...
[coord, elem, dof, adof, E, rho, f] = data_michell_a();
[coord, elem, dof, adof, E, rho, f] = data_michell_c();
m = size(elem,1);
cathetus = coord(elem(:,2),:)-coord(elem(:,1),:); % nel x 3
elem_length = vecnorm(cathetus'); % 1 x nel

%draw_truss(coord,elem,ones(m,1),[2 1]);
...
draw_truss(coord,elempos,xpos,[1 1]);
