...
[coord, elem, dof, adof, E, rho, f] = data_cantilever_a1();
%[coord, elem, dof, adof, E, rho, f] = data_cantilever_b1();
m = size(elem,1);
...
n = size(f,1);
Kdx = cell(m,1);
B = mat_B_volume(E*ones(m,1),elem,coord,dof,adof);
for i = 1:m
    Kdx{i} = B(:,i)*B(:,i)';
...
fprintf('Comparing LPsol with cpas.\n')
load("LP_sol","xLP")
[[xLP;mincompl(Kfun(xLP),f)] xsol]

xsol2 = xsol;
save("ex2compl_wo_freq","xsol2")
