function [coord, elem, dof, adof, E, rho, f] = data_mbbbeam(nat_numb)
    nx = 3*nat_numb + 1;
    ny = nat_numb + 1;
    nz = 1;
    index = 1;
    ...
    fdof = [(nx-1)*3+2 (nx-1)*3+3];
    for i = 1:nn
        offs = (i-1)*3;
        if coord(i,1) == 0
            fdof = [fdof offs+1 offs+3];
        end
    ...
    f = zeros(ndof,1);
    upperleftnode = nx*nat_numb + 1;
    f((upperleftnode-1)*3+2,1) = -1;%1e3; % N
    f = f(adof,:);
end